...
const Form = ({ data, errors, loading, onChange, onSubmit, onReset, dates, budgets=[] }) => {
	const [_dates, setDates] = useState({})

	useEffect(() => {
		if (dates) setDates(dates)
		else if (data.budgetId && budgets && budgets.length > 0) {
			const budget = budgets.find(budget => budget.id === data.budgetId)
			if (budget) {
				setDates({
			...
			<div className="w-full md:flex md:flex-wrap">
				{budgets && budgets.length > 0 && (
					<div className="mb-4 md:px-2 md:w-1/2 lg:mb-5 lg:px-4">
						<Select
							...
							onChange={(e) => {
								if (e.target.value !== UNCATEGORIZED_ID) {
									const budget = budgets.find(budget => budget.id === e.target.value)
									if (budget) {
										setDates({
											start_date: budget.start_date,
											end_date: budget.end_date
				...
				<div className="mb-4 md:px-2 md:w-1/2 lg:mb-5 lg:px-4">
					<Input
						...
						labelSize="text-sm tracking-wider md:text-base"
						onChange={onChange}
						min={_dates?.start_date ? new Date(_dates.start_date).toLocaleDateString('en-CA') : undefined}
						max={_dates?.end_date ? new Date(_dates.end_date).toLocaleDateString('en-CA') : undefined}
						name="date"
						padding="px-4 py-3"
